...
variable "OrgVcdNetworkName" {
	type = string
	default = "OrgNet-direct-$OutputOrg"
	} 
variable "VappName" {
	type = string
	default = "vApp_$OutputClt"
    }
variable "VappNetworkName" {
	type = string
	default = "vAppNet-$OutputClt"
	} 
variable "Vm1Name" {
	type = string
	default = "DC"
...
$plugin.add_Click({
start-Process -FilePath "C:\Windows\System32\cmd.exe" -ArgumentList "/k","$temp\Terraform\init.cmd" -Verb RunAs -Wait | Out-Null
